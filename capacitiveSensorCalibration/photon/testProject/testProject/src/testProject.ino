...
unsigned int movingAverage(unsigned int masterCaptureArrayInput[120])
{
    ...
    return (sum / masterCaptureArraySize);
}

unsigned int movingStandardDeviation(unsigned int masterCaptureArrayInput[masterCaptureArraySize], unsigned int average)
{

    unsigned int sum = 0;
    unsigned int temp = 0;

    for (unsigned int index = 0; index < masterCaptureArraySize; index++)
    {
